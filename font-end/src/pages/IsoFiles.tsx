...
const IsoFiles: React.FC = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      try {
        ...
        const isoOnly = data.filter(
          (r) => r.url && r.url.toLowerCase().includes('.iso')
        );
        setResources(isoOnly);
      } catch (err) {
        console.error('Error fetching ISO files:', err);
        setError('Không thể tải danh sách ISO. Vui lòng thử lại sau.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleDownload = async (resource: Resource) => {
    const filename =
      ...
      (resource.name ? `${resource.name}.iso` : undefined);
    await ResourceService.downloadResource(resource.id, filename);
  };

  const breadcrumb = { title: 'ISO Files', route: '/file-types/iso' };
      ...
      <div className="container mx-auto px-4 py-6">
        ...
        <div className="mb-6">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <h2 className="text-xl font-semibold text-gray-900">ISO Files</h2>
              <p className="text-sm text-gray-500 mt-1">
                Danh sách các tài nguyên có đuôi .iso trong hệ thống
              </p>
            </div>
            <Link
              to="/resources"
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200">
          {loading ? (
            ...
            <div className="py-12 px-4 text-center">
              <p className="text-sm text-gray-500">Chưa có file ISO nào trong hệ thống.</p>
            </div>
          ) : (
