...
const ApkFiles: React.FC = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      try {
        ...
        const apkOnly = data.filter(
          (r) => r.url && r.url.toLowerCase().includes('.apk')
        );
        setResources(apkOnly);
      } catch (err) {
        console.error('Error fetching APK files:', err);
        setError('Không thể tải danh sách APK. Vui lòng thử lại sau.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const breadcrumb = { title: 'APK Files', route: '/file-types/apk' };
      ...
      <div className="container mx-auto px-4 py-6">
        ...
        <div className="mb-6">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <h2 className="text-xl font-semibold text-gray-900">APK Files</h2>
              <p className="text-sm text-gray-500 mt-1">
                Danh sách các tài nguyên có đuôi .apk trong hệ thống
              </p>
            </div>
            <Link
              to="/resources"
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200">
          {loading ? (
            ...
            <div className="py-12 px-4 text-center">
              <p className="text-sm text-gray-500">Chưa có file APK nào trong hệ thống.</p>
            </div>
          ) : (
