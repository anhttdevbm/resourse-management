...
const KNOWN_EXTENSIONS = ['.apk', '.exe', '.iso'];
...
const OtherFiles: React.FC = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      try {
        ...
        const othersOnly = data.filter((r) => {
          if (!r.url) return false;
          const lower = r.url.toLowerCase();
          return !KNOWN_EXTENSIONS.some((ext) => lower.includes(ext));
        });
        setResources(othersOnly);
      } catch (err) {
        console.error('Error fetching Other files:', err);
        setError('Không thể tải danh sách file khác. Vui lòng thử lại sau.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const breadcrumb = { title: 'Other Files', route: '/file-types/others' };
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                Danh sách các tài nguyên không thuộc loại APK, EXE, ISO
              </p>
            </div>
            <Link
              to="/resources"
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200">
          {loading ? (
            ...
            <div className="py-12 px-4 text-center">
              <p className="text-sm text-gray-500">Chưa có file nào thuộc loại khác trong hệ thống.</p>
            </div>
          ) : (
