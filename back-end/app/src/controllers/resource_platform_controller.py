...
def update_resource_platform(
    resource_platform_id: str,
    resource_platform_update: ResourcePlatformUpdate,
...
@resource_forms_routers.get("/resource_platforms/{resource_platform_id}")
def read_resource_platform_by_id(
    resource_platform_id: str,
    db_session: Session = Depends(get_db_session),
    _: Tuple[User, str] = Depends(user_service.get_current_user),
) -> ResponseObject:
...
def create_resource_platform(
    resource_platform_create: ResourcePlatformCreate,
    db_session: Session = Depends(get_db_session),
    _: User = Depends(require_manage_catalog),
) -> ResponseObject:
    """Create a resource platform."""
    data = resource_form_service.create(db_session, resource_platform_create)
    return ResponseObject(data=row2dict(data), code="BE0000")


@resource_forms_routers.delete("/resource_platforms/{resource_platform_id}")
def delete_resource_platform(
    resource_platform_id: str,
    db_session: Session = Depends(get_db_session),
    _: User = Depends(require_manage_catalog),
) -> ResponseObject:
