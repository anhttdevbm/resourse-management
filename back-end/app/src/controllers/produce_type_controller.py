"""Define produce type controller."""
...
def update_produce_type(produce_type_id: str, produce_type_update: ProductTypeUpdate, db_session: Session = Depends(get_db_session) # noqa
...
def read_produce_types(db_session: Session = Depends(get_db_session) # noqa
                       , _: Tuple[User, str] = Depends(user_service.get_current_user)) -> ResponseObject: # noqa
...
def read_produce_type_by_id(produce_type_id: str, db_session: Session = Depends(get_db_session),
                            _: Tuple[User, str] = Depends(user_service.get_current_user)) -> ResponseObject: # noqa
    """Get produce type by id."""
    data = produce_type_service.get(db_session, produce_type_id)
    return ResponseObject(data=row2dict(data), code="BE0000")


@produce_types_routers.post("/produce_types")
def create_produce_type(produce_type_create: ProductTypeCreate, db_session: Session = Depends(get_db_session),
                          _: Tuple[User, str] = Depends(user_service.get_current_user)) -> ResponseObject: # noqa
    """Define creat a produce type."""
    data = produce_type_service.create(db_session, produce_type_create)
    return ResponseObject(data=row2dict(data), code="BE0000")


@produce_types_routers.delete('/produce_types/')
def delete_produce_type(produce_type_id: str, db_session: Session = Depends(get_db_session),
                          _: Tuple[User, str] = Depends(user_service.get_current_user)) -> ResponseObject: # noqa
